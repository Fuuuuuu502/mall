...
public class OrderServiceImpl implements OrderService {
    ...
    //查询收藏的skuId
    @Override
    public List<String> findCollet(String username) {
        List<String> colletSkuIds = colletMapper.findColletSkuId( username );
            return colletSkuIds;
    }

    @Override
    public boolean collet(String username, String skuId) {
        com.changgou.order.pojo.Collet collet = new com.changgou.order.pojo.Collet();
        collet.setUsername( username );
        collet.setCreated( new Date(  ) );
        collet.setSpuId( skuId );

        Example example = new Example( Collet.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo( "username",username );
        criteria.andEqualTo( "skuId",skuId );
        List<Collet> collets = colletMapper.selectByExample( example );
        if (collets.size()>0){//说明已经收藏过了
            return false;
        }
        colletMapper.insertSelective( collet );
        return true;
    }
    ...
    //根据用户查询所有订单
    @Override
    public List<Map<String, Object>> findAllOrder(String username){
            ...
            for (Order order : orderList) {
                ...
                if (orderItems != null && orderItems.size() > 0) {
                    for (OrderItem orderItem : orderItems) {
                        ...
                        map.put("image", orderItem.getImage());
                        map.put("price", orderItem.getPrice());
                        map.put("num", orderItem.getNum());
                        map.put("pay_money", orderItem.getPayMoney());
                        map.put("consign_status", order.getConsignStatus());
         ...
         */
        @Override
        public List<OrderItem> findUnshippedOrder (String username){
            ...
            for (Order order : orderList) {
                ...
                criteria1.andEqualTo("orderId", orderId);
                List<OrderItem> orderItemList = orderItemMapper.selectByExample(example2);
                for (OrderItem orderItem : orderItemList) {
        ...
        @Override
        public List<OrderItem> findUnreceivedOrder (String username){
         ...
         */
        @Override
        public UnrateResult<OrderItem> findUnrateOrder (String username,int currentPage, int pageSize){
            ArrayList<OrderItem> arrayList = new ArrayList();
            //根据用户名查询未收货订单表
            Example example = new Example(Order.class);
            Example.Criteria criteria = example.createCriteria();
            criteria.andEqualTo("username", username);
            criteria.andEqualTo("buyerRate", "0");          // 0：未评价        1： 已评价
            criteria.andEqualTo("consignStatus", "2");      // 发货状态 0:未发货 1:已发货 2:已送达
            List<Order> orderList = orderMapper.selectByExample(example);
            Page<Order> page = PageHelper.startPage(currentPage, pageSize).doSelectPage(() -> orderMapper.selectByExample(example));
            List<Order> result = page.getResult();


            UnrateResult<OrderItem> orderPageResult = new UnrateResult<>();
            ...
            for (Order order : result) {
                ...
                HashMap stringListHashMap = new HashMap();
                stringListHashMap.put("orderId", order.getId());
                stringListHashMap.put("orderItems", orderItems);
                hashMapArrayList.add(stringListHashMap);
            }

            orderPageResult.setTotal(page.getTotal());  //设置总记录数
            orderPageResult.setRows(hashMapArrayList);

            return orderPageResult;
        }

        @Override
        public Map findOrderDetailById (String orderId){
            ...
            listHashMap.put("order", order);
            listHashMap.put("orderItemList", orderItemList);
        ...
        public List<Map<String, Object>> waitPay (String username){
            ...
            if (orderList != null && orderList.size() >= 0) {
                for (Order order : orderList) {
                    //封装待付款数据
                    if ("0".equals(order.getPayStatus()) && "0".equals(order.getConsignStatus())) {
                        String orderId = order.getId();
                        Example example1 = new Example(OrderItem.class);
                        Example.Criteria criteria1 = example1.createCriteria();
                        criteria1.andEqualTo("orderId", orderId);
