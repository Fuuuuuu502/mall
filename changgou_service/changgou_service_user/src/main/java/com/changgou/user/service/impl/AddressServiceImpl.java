...
public class AddressServiceImpl implements AddressService {
    ...
    public void defaultModification(Integer id) {
        //1.首先应该将原有的默认收件人状态设置为0
        //1.1查询出原有收件人
        Address address = this.findById(id);
        String username = address.getUsername();
        Address modaddress = addressMapper.queryDefaultRecipients(username);
        //1.2修改原默认收件人状态
        addressMapper.defaultRecipient(modaddress.getId());
        //2.再进行修改默认收件人状态
        addressMapper.defaultModification(id);
    }

    //根据用户名称查出默认收件人
    @Override
    public Address queryDefaultRecipients(String username) {
        return addressMapper.queryDefaultRecipients(username);
    }

    //修改原默认人状态为0
    @Override
    public void defaultRecipient(Integer id) {
        addressMapper.defaultRecipient(id);
    }
